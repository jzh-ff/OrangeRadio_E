...
| 8 | **演唱会现场** | 荧光棒灯海 + LED 色浪 + 鼓点闪光 + 3 道聚光灯 |

另有独立于数值预设的「胶片电台」特殊卡片（`10-shell/06-film-radio.js`，全屏黑胶播放器，切换数值预设自动退出）。

### 6.4 主题系统

CSS 变量集中在 `public/css/index.css` 多个 `:root` 块（灰阶/品牌色/玻璃材质/图标色）。主题切换 = 运行时改写 CSS 变量（`07-fx/02-accent-background-controls.js`），无预设皮肤机制。胶片电台独立样式在 `public/css/film-radio.css`（`body.film-radio` 命名空间）。
