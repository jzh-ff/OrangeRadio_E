/* =========================================================================
   OrangeSea · 天气电台（weather）
   -------------------------------------------------------------------------
   从原 server.js 拆出：open-meteo 天气获取、情绪电台（mood）构建、
   天气电台歌曲聚合与排序。依赖 netease-playback / netease-mappers /
   search 路由的 handleSearch / utils / context。
   ========================================================================= */
'use strict';

const { playlist_track_all, playlist_detail } = require('NeteaseCloudMusicApi');
const { requestJson, clampNumber } = require('../utils');
const { UA, getUserCookie, OPEN_METEO_FORECAST_URL, OPEN_METEO_GEOCODE_URL, WEATHER_IP_LOCATION_URL, WEATHER_DEFAULT_LOCATION } = require('../context');
const { mapSongRecord } = require('./netease-mappers');
const { handleSongUrl } = require('./netease-playback');
const { handleSearch } = require('./netease-search');
...
module.exports = {
  ...
  uniqueSongsByKey,
  tagWeatherPoolSongs,
  fetchWeatherPlaylistSongs,
  filterLikelyPlayableWeatherSongs,
  isLowSignalWeatherSong,
  scoreWeatherSong,
  weatherArtistKey,
  weatherTitleKey,
  uniqueWeatherTitles,
  diversifyWeatherSongs,
  orderWeatherSongs,
  buildWeatherRadio,
};
