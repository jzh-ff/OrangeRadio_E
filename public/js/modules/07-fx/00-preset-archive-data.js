...
var presetMeta = [
  ...
  { name: '演唱会现场', nameHtml: '演唱会现场 <span class="pc-name-en">Concert Live</span>', desc: '聚光灯 · 荧光棒灯海' },
];
var presetIcons = [
  ...
  '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M6 4.5l4.5 14"/><path d="M18 4.5l-4.5 14"/><path d="M5 19h14"/><circle cx="12" cy="4.5" r="1.1" fill="currentColor"/></svg>',
];
...
var USER_FX_ARCHIVE_STORE_KEY = 'orangesea-user-fx-archives-v1';
var USER_FX_ARCHIVE_EXPORT_TYPE = 'orangesea-user-fx-archive';
var USER_FX_ARCHIVE_SCHEMA = 1;
var USER_FX_SHARE_PREFIX = 'OS2';
...
function extractUserFxShareCode(text) {
  ...
  if (/^OS2:[0-9]+\.[A-Za-z][A-Za-z0-9_-]+\.[A-Za-z0-9]+$/.test(direct)) return direct;
  var match = text.match(/OS2:[0-9]+\.[A-Za-z][A-Za-z0-9_-]+\.[A-Za-z0-9]+/);
...
async function copyUserFxArchiveShareCode(index) {
  ...
  try {
    ...
    if (copied) {
      ...
      window.prompt('复制这段 OS2 短代码', code);
      showToast('已打开完整短码');
    }
  } catch (e) {
    showToast('短码生成失败');
...
function renderUserFxArchives() {
  ...
  var toolbar =
    ...
    '<div class="user-archive-note">主入口使用 OS2 短代码复制/粘贴；旧 JSON 仍可拖拽或作为兼容备份导入。</div>' +
  ...
  var shareBox =
    ...
    '<textarea id="user-archive-share-input" class="user-archive-share-input" spellcheck="false" placeholder="把 OS2 短代码粘到这里，也兼容旧 JSON 存档" oninput="updateUserFxArchiveShareDraft(this.value)" onkeydown="handleUserFxArchiveShareInputKey(event)">' + escHtml(userFxArchiveShareDraft) + '</textarea>' +
